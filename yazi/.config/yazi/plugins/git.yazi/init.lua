...
local M = {
  setup = function(st, opts)
    ...
    local function header_git(self)
      return (st.git_branch and st.git_branch ~= '')
          and ui.Line({ ui.Span(' <' .. st.git_branch .. st.git_is_dirty .. '>'):fg('green') })
        or ui.Line({})
    end
  ...
  entry = function(_, args)
    ...
    local git_branch
    local file = io.popen('git symbolic-ref HEAD 2> /dev/null', 'r')
    if not file then
      return
    end
    output = file:read('*a')
    ...
    local file = io.popen('git status --ignored -s --ignore-submodules=dirty 2> /dev/null', 'r')
    if not file then
      return
    end
    output = file:read('*a')
